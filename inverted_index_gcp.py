...
class InvertedIndex:
    ...
    def add_doc(self, doc_id, tokens):
        ...
        side-effects).
    """
        self._DL[doc_id] = self._DL.get(doc_id, 0) + (len(tokens))
    ...
    def write_dct(self, base_dir, name):
        """ Write the in-memory index to disk. Results in the file:
        (1) `name`.pkl containing the global term stats (e.g. df).
        """
        self._write_dct(base_dir, name)

    def _write_dct(self, base_dir, name):
        file_to_write = open(Path(base_dir) / f'{name}.pkl', 'wb')  #
        pickle.dump(self.dct, file_to_write)
    ...
    def read_posting_list(self, w, path):
        '''
        ...
        with closing(MultiFileReader()) as reader:
            if w in self.df:
                ...
                locs = [(path + loc[0], loc[1]) for loc in locs]
